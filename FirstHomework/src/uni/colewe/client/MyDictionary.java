...
public class MyDictionary {
	...
	private static Map<String, MyDictionaryEntry> ruToEng = new HashMap<String, MyDictionaryEntry>();
	private static Map<String, List<String>> engToRu = new HashMap<String, List<String>>();
	...
	public String lookup(String query, boolean reverse, boolean startswith, boolean highlight) {
		Set<String> results = new HashSet<String>();
		if (reverse) {
			if (startswith) {
				for(String key : engToRu.keySet()){
					if(key.startsWith(query)){
						results.addAll(engToRu.get(key));
					}
				}
			} else {
				if(engToRu.get(query) != null){
					results.addAll(engToRu.get(query));
				}
			}
		} else {
			if (startswith) {
		...
		return getHTMLString(results, query, reverse, startswith , highlight);
	}
